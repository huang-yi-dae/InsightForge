            ...
            {skeleton.points.map((p, i) => (
              <li key={i} data-el="skeleton-point">
                ...
                <button
                  ...
                  disabled={left <= 0 || busyHeading === p.heading}
                  title={left <= 0 ? t("write.expandDisabled") : undefined}
                  onClick={async () => {
                    setBusyHeading(p.heading);
                    await onExpand(p.heading);
                    setBusyHeading(null);
                  }}
                  className="mt-2 ml-5 inline-flex items-center gap-1.5 rounded-md border border-primary/40 bg-primary/8 px-2.5 py-1 text-xs font-medium text-primary transition-colors hover:bg-primary/15 disabled:cursor-not-allowed disabled:border-border disabled:bg-transparent disabled:text-muted-foreground disabled:opacity-60"
                >
                  {busyHeading === p.heading ? (
                  ...
                  {busyHeading === p.heading
                    ? t("write.expanding")
                    : left <= 0
                      ? t("write.expandUsedUp")
                      : t("write.requestExpand")}
                </button>
              </li>
            ))}
          </ol>
