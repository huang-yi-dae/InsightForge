...
import { BookOpen, CheckCircle2, ChevronRight, Copy, Download, FileDown } from "lucide-react";
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <Link
                    href={`/library/${w.id}`}
                    data-el="library-open"
                    className="zz-serif group inline-flex items-center gap-1 text-lg font-semibold leading-snug text-foreground transition-colors hover:text-primary"
                  >
                    {w.title}
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  ...
                  <span className="ml-auto flex items-center gap-1.5">
                    ...
                    <button
                      data-el="library-download-pdf"
                      onClick={() =>
                        printWritingToPdf(
                          w,
                          t("library.pdfMeta", {
                            date: new Date(w.publishedAt).toLocaleDateString(),
                            count: w.userWords + w.aiWords,
                            ratio: humanShare,
                          }),
                        )
                      }
                      className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 font-medium text-foreground/70 hover:text-foreground"
                    >
                      <FileDown className="h-3.5 w-3.5" /> {t("library.downloadPdf")}
                    </button>
                  </span>
                </div>
              </article>
