...
import { KnowledgeImport } from "@/components/zhizhi/knowledge-import";
import { Button } from "@/components/ui/button";
import { useZhizhi } from "@/lib/zhizhi/store";
import { clusterLabel } from "@/lib/zhizhi/cluster-label";
import { rankGapsByIdentity } from "@/lib/zhizhi/gap-ranking";
import { assessGapReadiness } from "@/lib/zhizhi/gap-readiness";
import { cn } from "@/utils/utils";

function GapsInner() {
  const { t } = useTranslation();
  const router = useRouter();
  const { gaps, clusters, fragments, fragmentsForGap, ensureDraftForGap, discoverNewGapsAI, identity } = useZhizhi();
  const activeGaps = gaps.filter((g) => g.status !== "published");
  // AI 选题增强：用（可被 AI 自动更新的）身份档案对候选选题重排，命中深耕话题者靠前。
  const ranked = useMemo(() => rankGapsByIdentity(activeGaps, identity), [activeGaps, identity]);
  ...
  async function recon() {
    ...
    try {
      const { added, usedAi, reason } = await discoverNewGapsAI(3);
      if (reason) {
        // 用户配了 key 但直连失败：给出可行动的提示，而不是让他以为"没生效"。
        toast.warning(t(`gaps.reconErr.${reason}`));
      } else if (added > 0) {
        toast.success(t(usedAi ? "gaps.reconDoneAi" : "gaps.reconDone", { count: added }));
      } else {
        toast.info(t("gaps.reconNone"));
      }
    } finally {
    ...
    <div data-el="gaps" className="mx-auto w-full max-w-6xl px-4 py-6 md:px-8 md:py-10">
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <KnowledgeImport />
          <Button variant="outline" size="sm" data-el="gaps-recon" onClick={recon} disabled={reconLoading} className="gap-1.5">
            {reconLoading ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
              <RefreshCw className="h-3.5 w-3.5" />
            )}
            {reconLoading ? t("gaps.reconLoading") : t("gaps.recon")}
          </Button>
        </div>
      </div>
              ...
              {clusters.map((c, i) => {
                ...
                return (
                  <div
                    ...
                    style={pos}
                  >
                    <span className="line-clamp-2">{clusterLabel(c.label, t)}</span>
            ...
            {ranked.map(({ gap, matched }) => (
              ...
              >
                ...
                <span className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  {(() => {
                    const r = assessGapReadiness(gap, fragments);
                    return (
                      <span
                        data-el="gap-readiness-badge"
                        className={cn(
                        ...
                        title={t(`gaps.readiness.${r.reasonKey}`, { evidence: r.evidence, fresh: r.fresh })}
                      >
                        {r.level === "ready" ? `✅ ${t("gaps.readyLabel")}` : `⚠️ ${t("gaps.thinLabel")}`}
                      </span>
                    );
                  })()}
          ...
          >
            ...
            {selected ? (
              <div className="mt-3 space-y-4">
                ...
                {selected.aiReason && (
                  <div
                    data-el="gaps-ai-reason"
                    className="rounded-lg border border-accent/30 bg-accent/8 px-3 py-2 text-xs leading-relaxed text-accent"
                  >
                    ✨ {t("gaps.aiReasonLabel")}：{selected.aiReason}
                  </div>
                )}
                <div>
                    ...
                    {selected.clusterIds.map((cid) => (
                      ...
                      >
                        {(() => {
                          ...
                          return c ? clusterLabel(c.label, t) : null;
                        })()}
                      </span>
                    ))}
                  </div>
